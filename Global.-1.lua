...
GITHUB_BASE_URL = "https://raw.githubusercontent.com/maxm4004/Tabletop-Simulator-Lua/refs/heads/main/Json/"
...
function onObjectSpawn(obj)
    Wait.frames(function()
        ...
        if obj.getName() == "Ground" and ground_scala_salvata then
            obj.setScale(ground_scala_salvata)
...
function scegliElemento(elementi)
    local peso_totale = 0
    for _, e in ipairs(elementi) do peso_totale = peso_totale + (e.peso or 1) end
    local r = math.random(1, peso_totale)
    local acc = 0
    for _, e in ipairs(elementi) do
        acc = acc + (e.peso or 1)
...
-- ------------------------------------------------------------
function groundCentra()
    local table_obj  = getByName("Table")
    local ground_obj = getByName("Ground")
    if not table_obj  then printToAll("[GROUND] 'Table' non trovato!",  {r=1,g=0.3,b=0.3}) return end
    if not ground_obj then printToAll("[GROUND] 'Ground' non trovato!", {r=1,g=0.3,b=0.3}) return end
...
function spawnGroundButtons()
    ...
    local ground_obj = getByName("Buttons")
    if not ground_obj then
        printToAll("[Buttons] Oggetto 'Ground' non trovato!", {r=1,g=0.3,b=0.3})
        return
    end
